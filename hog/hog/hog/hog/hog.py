...
def swine_align(player_score, opponent_score):
    ...
    if(player_score==0 or opponent_score==0):
        ...
        while player_score%opponent_score != 0:
            t = opponent_score
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0<goal and score1<goal:
        if who:#判断到谁掷骰子
            ...
            if not extra_turn(score1,score0):#判定是否再来一局
                who=other(who)
        else:
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def compare_say(current_score,opponent_score):
        son_running_high = running_high #son_running_highson_running_high接受外部函数的参数
        if who:
            gain = opponent_score-last_score
        else:
            gain = current_score-last_score
        if gain > son_running_high: #大于才打印
            son_running_high = gain
            print(son_running_high,"point(s)! The most yet for Player",who)
        if who:
            return announce_highest(who,opponent_score,son_running_high)
        else:
            return announce_highest(who,current_score,son_running_high)
    return compare_say
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    def averaged_function(*args):
        ...
        for i in range(0,trials_count):
            averaged_results += original_function(*args)
        averaged_results /= trials_count
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    max_score_rolls = 1 #存最大的分数所对应的骰子数
    # 调用make_averaged求平均值
    averaged_dice = make_averaged(roll_dice,trials_count)   
    max_scores = averaged_dice(1,dice)
    # 对1到10次比较
    for i in range(2,11):
        this_time_scores = averaged_dice(i,dice)
        if this_time_scores > max_scores:
            max_scores = this_time_scores
            max_score_rolls = i
